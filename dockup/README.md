...
If you prefer to use an IAM Role, set the following environment variable:

```
-e AWS_USE_SERVICE_TASK_ROLE=true
```
...
--env-file env.txt \
--volumes-from mysql \
--name dockup joshes/dockup:latest
```
...
The equivalent `docker run` commandline:

```
docker run --rm \
    -e AWS_ACCESS_KEY_ID=<key_here>
    -e AWS_SECRET_ACCESS_KEY=<secret_here>
    -e AWS_DEFAULT_REGION=us-east-1
    -e BACKUP_NAME=mysql
    -e PATHS_TO_BACKUP=/etc/mysql /var/lib/mysql
    -e S3_BUCKET_NAME=docker-backups.example.com
    -e RESTORE=false
    joshes/dockup:latest
```
